  ...
  for (const file of files) {
    ...
    try {
      ...
      if (query && query.trim()) {
        ...
        const labelMatch = Array.isArray(email.labels) && email.labels.some(l => l.toLowerCase().includes(q));
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (event.start) {
        ...
        const endTime = event.end ? new Date(event.end).getTime() : startTime;

        if (startDate) {
          const sTime = new Date(startDate).getTime();
          if (!isNaN(sTime) && endTime < sTime) matches = false;
        }
        if (endDate) {
